...
export class Picker {
    constructor(camera, scene, config = {}) {
        ...
        // 线型对象拾取阈值（世界坐标），Line2 原生 raycast 会使用此值
        this.raycaster.params.Line.threshold = config.lineThreshold || 0.5;
    }
    ...
    pick(x, y, targets) {
        ...
        // 对于线型对象，使用 Line2 原生 raycast（基于屏幕空间线宽）
        const lineTargets = targets.filter(t =>
            t.userData?.objectType === 'segment' ||
            t.userData?.objectType === 'line' ||
            t.userData?.objectType === 'ray'
        );

        if (lineTargets.length > 0) {
            const intersects = this.raycaster.intersectObjects(lineTargets, true);
